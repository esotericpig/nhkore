...
Gem::Specification.new do |spec|
  ...
  spec.add_dependency 'bimyou_segmenter'    ,'~> 1.2'  # Splitting Japanese sentences into words.
  spec.add_dependency 'cri'                 ,'~> 2.15' # CLI commands/options.
  spec.add_dependency 'down'                ,'~> 5.4'  # Downloading files (GetCmd).
  spec.add_dependency 'highline'            ,'~> 3.1'  # CLI input/output.
  spec.add_dependency 'http-cookie'         ,'~> 1.0'  # Parsing/Setting cookies [(Bing)Scraper].
  spec.add_dependency 'japanese_deinflector','~> 0.0'  # Unconjugating Japanese words (dictionary form).
  spec.add_dependency 'nokogiri'            ,'~> 1'    # Scraping/Hacking.
  spec.add_dependency 'psychgus'            ,'~> 1.3'  # Styling Psych YAML.
  spec.add_dependency 'public_suffix'       ,'~> 6.0'  # Parsing URL domain names.
  spec.add_dependency 'rainbow'             ,'~> 3.1'  # CLI color output.
  spec.add_dependency 'rss'                 ,'~> 0.3'  # Scraping [(Bing)Scraper].
  spec.add_dependency 'rubyzip'             ,'~> 2.4'  # Extracting Zip files (GetCmd).
  spec.add_dependency 'tiny_segmenter'      ,'~> 0.0'  # Splitting Japanese sentences into words.
